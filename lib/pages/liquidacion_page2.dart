...
import '../repositories/repo_update_liquidacionD2.dart';
import '../repositories/repo_update_liquidacion_m2.dart';
import '../repositories/security_data.dart';
import '../utils/colors.dart';
import '../utils/dimens.dart';
import '../utils/iconos.dart';

class LiquidacionPage2 extends StatefulWidget {
  LiquidacionPage2({Key? key});
...
class _LiquidacionPageState extends State<LiquidacionPage2> {
  String propietario = "";
  ...
  final TextEditingController ida = TextEditingController(text: '0');
  final TextEditingController vuelta = TextEditingController(text: '0');
  ...
  Widget _buildHeaderRow() {
    return Column(
      ...
      children: [
        Row(
          children: const [
            _HeaderCell("#", width: 25),
            _HeaderCell("Horario", width: 55),
            _HeaderCell("Ruta", width: 35),
            _HeaderCell("Frec", width: 30),
            _HeaderCell("Sub.", width: 45),
            _HeaderCell("Baj.", width: 40),
            _HeaderCell("Tot.", width: 70),
            _HeaderCell("", width: 37),
          ],
        ),
  ...
  Widget _buildDataRow(int index) {
    ...
    return Column(
      children: [
        Row(
          children: [
            Container(width: 25, child: _CellText('${item.numVuelta}')),
            Container(
              width: 55,
              child: _CellText(
                '${item.horaSalida}\n${item.horaLLegada}',
            ...
            Container(
              width: 30,
              child: _CellText('${item.LetrRuta}', fontSize: 15, isBold: true),
            ),

            Container(
              width: 30,
              child: _CellText('${item.LetrFrec}', fontSize: 15, isBold: true),
            ),
            Container(
              width: 45,
              child: _CellText('${item.dineroIda}', fontSize: 14, isBold: true),
            ),
            Container(
              width: 40,
              child: _CellText(
            ...
            /*  Container(
              ...
              child:
                  item.estado_ld == 1
                      ...
                      : _EditableCellValor(controller: _valorImputCntroler),
            ), */
            Container(
              width: 70,
              child: _CellText(
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              _buildConfirmRow("Ruta:", item.descRuta),
              //_buildConfirmRow("Descripción:", item.descRuta),
              _buildConfirmRow(
                "Horario:",
                '${item.horaSalida} - ${item.horaLLegada}',
              ),
              _buildConfirmRow("Frecuencia:", '${item.DescFrec}'),
              const SizedBox(height: 8),
              Divider(height: 2, color: Colors.black),
              const SizedBox(height: 8),
              _buildRowPar('Ida', ida, 'Vuelta', vuelta),
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    final Map<String, dynamic> datos = {
                      ...
                      "dineroIda": double.tryParse(ida.text) ?? 0.0,
                      "dineroVuelta": double.tryParse(vuelta.text) ?? 0.0,
                    };
                    ...
                    _updateLiquidacionD(
                      ...
                      double.tryParse(ida.text) ?? 0.0,
                      double.tryParse(vuelta.text) ?? 0.0,
                    );
  ...
  Widget _buildTotalRow(double totalValor) {
    return Container(
      child: Row(
        children: [
          Container(width: 25, child: _CellText('')),
          Container(
            width: 55,
            child: _CellTextTotal('TOTAL'),
          ), // Espacio vacío para el nombre
          Container(width: 30, child: _CellTextTotal('')),
          Container(width: 30, child: _CellTextTotal('')),
          Container(width: 45, child: _CellTextTotal('')),
          Container(width: 40, child: _CellTextTotal('')),

          Container(
            width: 70,
            child: _CellTextTotal('$totalValor'),
          ), // Muestra el total Valor
          Container(width: 40), // Espacio vacío para el botón
        ],
      ),
    );
  }
  ...
  Widget _buildInput(String label, TextEditingController controller) {
    return Row(
      ...
      children: [
        ...
        Expanded(
          child: Padding(
            padding: const EdgeInsets.only(right: 8), // 👈 margen derecho aquí
            child: Focus(
  ...
  void _updateLiquidacionD(
    ...
    ApiUpdateLiquidacionD2 servicio = ApiUpdateLiquidacionD2();
...
class _WidgetGastosState extends State<WidgetGastos> {
  ...
  void _updateLiquidacionM(
    ...
    final servicio = ApiUpdateLiquidacionM2();
    final resultadoApiUpdate = await servicio.fetchUpdateLiquidacionM2(
      idLiquidaM,
      unidad,
      fechaLiquidaM,
      gComida,
